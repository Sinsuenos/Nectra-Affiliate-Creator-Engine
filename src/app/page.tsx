    ...
    <>
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 py-16 sm:py-24 text-center">
          ...
          <motion.div className="mt-7 flex flex-wrap justify-center gap-x-6 gap-y-2 text-xs text-zinc-300" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.55 }}>
          ...
          <motion.div className="relative mx-auto mt-12 max-w-5xl rounded-2xl border border-white/15 bg-[#171b26]/90 p-3 sm:p-5 shadow-2xl shadow-black/40 backdrop-blur-sm" initial={{ opacity: 0, y: 30, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.7, delay: 0.65 }}>
            ...
            <div className="relative flex items-center gap-2 px-2 pb-3 text-left"><ScanLine className="h-4 w-4 text-electric" /><span className="font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-300">Watch the transformation</span></div>
            <div className="relative grid gap-2 sm:grid-cols-4">{previewStages.map((stage, i) => <div key={stage.label} className="relative"><motion.div className={`rounded-xl border px-3 py-3 text-left ${stage.tone}`} animate={{ y: [0, -2, 0] }} transition={{ duration: 3, repeat: Infinity, delay: i * 0.45, ease: "easeInOut" }}><div className="font-mono text-[9px] tracking-[0.18em] opacity-80">{stage.label}</div><div className="mt-1 text-xs sm:text-sm font-semibold text-white">{stage.value}</div></motion.div>{i < previewStages.length - 1 && <motion.div className="hidden sm:block absolute top-1/2 -right-2 z-10 h-px w-3 bg-gradient-to-r from-electric via-fuchsia-400 to-lime-300 shadow-[0_0_8px_rgba(34,184,255,0.5)]" animate={{ opacity: [0.35, 1, 0.35] }} transition={{ duration: 1.8, repeat: Infinity, delay: i * 0.35 }} />}</div>)}</div>
            <div className="relative mt-3 flex items-center justify-center gap-2 text-[10px] font-mono uppercase tracking-widest text-zinc-400"><Layers3 className="h-3 w-3 text-electric" /> One source, structured into usable decisions</div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-4 sm:px-6 pt-2 pb-24 sm:pb-32"><div className="grid gap-4 sm:grid-cols-3">{[{ tag: "SEE THE SYSTEM", title: "How Nectar Works", desc: "See the architecture behind the transformation, with shipped capabilities separated from the next layer.", href: "/modules" }, { tag: "SEE THE OUTPUT", title: "Sample Output", desc: "Explore a transparent example of the kind of structured content Nectar produces from one offer.", href: "/sample" }, { tag: "PROTECT THE COPY", title: "Compliance Scanner", desc: "Check generated content for platform-specific risk signals and get a safer rewrite when needed.", href: "/scanner" }].map((card, i) => <motion.div key={card.href} variants={fadeUp} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }}><Link href={card.href} className="group block h-full rounded-xl border border-white/12 bg-[#181c27] p-6 transition-all hover:-translate-y-1 hover:bg-[#202533] hover:border-electric/40 hover:shadow-lg hover:shadow-electric/10"><p className="font-mono text-[11px] uppercase tracking-widest text-electric mb-2 font-semibold">{card.tag}</p><h3 className="text-lg font-bold tracking-tight mb-2 text-white group-hover:text-electric transition-colors">{card.title}</h3><p className="text-sm text-zinc-300 leading-relaxed mb-4">{card.desc}</p><span className="inline-flex items-center gap-1 text-xs text-electric/80 group-hover:text-electric transition-colors font-semibold">Explore <ArrowRight className="h-3 w-3" /></span></Link></motion.div>)}</div></section>
    </>
  );
}
