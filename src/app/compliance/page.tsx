          ...
          <table className="w-full text-sm">
            ...
            <tbody>{PLATFORM_MATRIX.map((row, i) => {
              ...
              <tr key={row.id} className={`border-b border-border/30 last:border-0 border-l-2 ${colors.rowBorder} hover:bg-surface-raised/50 transition-colors`}>
                ...
                <td className="px-5 py-4 text-foreground/85 text-xs leading-relaxed max-w-xs">{row.bannedTriggers.join(", ")}</td>
                <td className="px-5 py-4 text-foreground/85 text-xs leading-relaxed max-w-xs">{row.safeApproach}</td>
                <td className="px-5 py-4 text-foreground/85 text-xs leading-relaxed max-w-xs">{row.notes}</td>
              </tr>
            ); })}</tbody>
          </table>
        </motion.div>
