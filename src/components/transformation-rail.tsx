    ...
    <motion.div className="hidden sm:flex items-center justify-center shrink-0 w-8 lg:w-12" variants={connectorVariants} style={{ originX: 0 }}>
      {bothLive ? (
        <div className="relative w-full h-px">
          <div className="absolute inset-0 bg-electric/35" />
          <motion.div className="absolute inset-y-0 left-0 w-1/2 bg-electric" animate={{ x: ["0%", "100%", "0%"] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut", delay: index * 0.4 }} />
        </div>
      ) : eitherLive ? (
        <div className="relative w-full h-px">
          <div className="absolute inset-0 bg-electric/20" />
          <motion.div className="absolute inset-y-0 left-0 w-1/3 bg-electric/60" animate={{ x: ["0%", "200%", "0%"] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: index * 0.5 }} />
        </div>
      ) : (
    ...
    <Tooltip open={open} onOpenChange={setOpen}>
      <TooltipTrigger asChild>
        <motion.div className="relative flex flex-col items-center gap-2 cursor-default select-none px-2 sm:px-0" variants={nodeVariants} onClick={() => setOpen(true)}>
          ...
          <span className="font-mono text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-full border border-electric/20 text-electric/50 bg-electric/[0.06]">Next</span>
...
function MobileConnector() {
  return <div className="flex sm:hidden items-center justify-center py-1"><ChevronRight className="h-3.5 w-3.5 text-muted-foreground/20 rotate-90" /></div>;
}

export function TransformationRail() {
  return (
    <motion.div className="mx-auto max-w-5xl px-4 sm:px-6 pb-20 sm:pb-28" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }}>
      ...
      <motion.div className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-center mb-12 sm:mb-16" variants={nodeVariants}>One Offer.<span className="text-electric"> Many Angles.</span> One Campaign.</motion.div>
      ...
      <div className="sm:hidden grid grid-cols-2 gap-x-4 gap-y-2 justify-items-center">
        {RAIL_STAGES.map((stage, i) => <div key={stage.id}><StageNode stage={stage} />{i < RAIL_STAGES.length - 1 && i % 2 === 1 && <MobileConnector />}</div>)}
      </div>
    </motion.div>
  );
}
...
function CompactNode({ stage, state }: { stage: RailStage; state: "done" | "active" | "next" | "locked" }) {
  const Icon = stage.icon;
  if (state === "locked") return <div className="flex items-center gap-1 px-1.5 sm:px-2 py-1 rounded-full border border-electric/10 text-electric/35 select-none"><Icon className="h-3 w-3 shrink-0" /><span className="font-mono text-[8px] sm:text-[10px] uppercase tracking-wider">{stage.label}</span><span className="font-mono text-[7px] sm:text-[8px] uppercase tracking-widest px-1 sm:px-1.5 py-px rounded-full border border-electric/15 text-electric/40 bg-electric/[0.05]">Next</span></div>;
  const styles = state === "active" ? "bg-electric/10 text-electric border border-electric/20" : state === "done" ? "text-electric/60" : "text-muted-foreground/50";
...
export function CompactTransformationRail({ currentStep }: CompactRailProps) {
  ...
  return <div><div className="flex items-center gap-0.5 sm:gap-2 mt-6 mb-1 flex-wrap">{RAIL_STAGES.map((stage, i) => { const stageState = getStageState(stage.id, currentStep); return <div key={stage.id} className="flex items-center">{i > 0 && <span className={`h-px w-1.5 sm:w-4 mr-0.5 sm:mr-1 ${stageState === "locked" ? "bg-electric/10" : stageState === "done" || stageState === "active" ? "bg-electric/40" : "bg-border/30"}`} />}{<CompactNode stage={stage} state={stageState} />}</div>; })}</div>{progressLabel && <p className="font-mono text-[10px] text-muted-foreground/60 mb-2">Current stage: <span className="text-electric/70">{progressLabel}</span></p>}</div>;
}
